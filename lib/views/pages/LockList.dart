...
class LockList extends StatelessWidget {
  const LockList({Key? key}) : super(key: key);
  ...
  Widget build(BuildContext context) {
    return SafeArea(
        child: SingleChildScrollView(
            ...
            child: Column(
              children: [
                Header(title: "Lock 列表",),
                Padding(padding: EdgeInsets.all(defaultPadding)),
                Search(onTap: (d,c) {
                  c.text = "";
                  print(d);
                },),
                PaginateList(header: "Lock List",
                    attributeNames: ['公链名称','代币地址','代币名称','代币简称','代币小数位','当前锁仓金额'],
                    attributes:['chain_name','token_address','name','symbol','decimals','current_amount'],
                    getData: (page,limit) async {
                  var res = await LocksController().getPaginateList(page: page, limit: limit);
                  return res;
                })
              ],
            )
