...
class AirdropLogsRandomExecute extends ExecuteTaskFactory {
  @override
  Task task;
  bool _lock = false;
  AirdropLogsRandomExecute(this.task);
    ...
    for (var i = 0; i < airdrops.length; i++) {
      ...
      if (indexArr.isNotEmpty) {
        ...
        task.web3.client.call(
            ...
            function: airdropContract.function('listOfRandom'),
            ...
            try {
              ...
              if (logs.isNotEmpty) {
                for (var j = 0; j < logs.length; j++){
                  var log = await AirdropLogs().first(where:"chain_id = ? and chain_index = ? and airdrop_address = ? and type = 2", whereArgs: [task.chainId,indexArr[j].toInt(),airdropAddress]);
                  if (log['is_parsed'] != 1) {
                    log['address'] = logs[j].toString();
                    log['rewards'] = airdrops[i]['random_rewards'];
                    if (isParsed(log)){
                      log["is_parsed"] = 1;
                    }
                    if (log['id'] as int > 0){
  ...
  Future<int> getStartIndexByAddress(String tokenAddress) async {
    return await AirdropLogs().count(where: "chain_id = ? and type = 2 and airdrop_address = ?",whereArgs: [task.chainId, tokenAddress]);
  }
  ...
  Future<int> getTotalCountByAddress(String tokenAddress) async {
    ...
    var total = await task.web3.client.call(
        contract: contract,
        function: contract.function('listOfRandom'), params: [BigInt.from(0), BigInt.from(1)]
    );
    return (total[0] as BigInt).toInt();
  }

  Future<List<BigInt>> getUnParsedChainIndexListByAddress(String tokenAddress) async {
    var list = await AirdropLogs().get(where: "chain_id = ? and is_parsed = 0 and airdrop_address = ? and type = 2",whereArgs: [task.chainId,tokenAddress],orderBy: "chain_index asc");
  ...
  Future<void> insertNewToDatabaseByAddress(int totalCount, int startIndex, String tokenAddress) async {
    if (totalCount > 0 && (totalCount - startIndex) > 0) {
      ...
      for (int a = startIndex; a < totalCount; a++) {
        batch.insert(AirdropLogs().tableName,AirdropLogs().fromMap({"chain_id":task.chainId,"chain_index":a,"airdrop_address":tokenAddress,"type":2}).toMap());
      }
      await batch.commit();
    }
  }
